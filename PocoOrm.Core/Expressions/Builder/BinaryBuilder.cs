...
namespace PocoOrm.Core.Expressions.Builder
{
        ...
        where TRight : ISqlBuilder
    {
        ...
        public string Build(ExpressionToSql parser, ISqlBuilder left, object comp, ISqlBuilder right, out DbParameter[] parameters)
        {
            return Build(parser, (TLeft)left, comp, (TRight)right, out parameters);
        }

        protected abstract string Build(ExpressionToSql parser, TLeft left, object comp, TRight right, out DbParameter[] parameters);
    }
}